...
- 使用枚举类型
- 使用触发器和触发器函数统一管理update_time，用默认值管理create_time和update_time，但是目前有时区的bug
- 使用 ip 类型
- 使用部分索引和逻辑删除
...
- 如果需要等值查询且要查询的字段为unique，则可以更进一步走redis的hash，前缀匹配走redis的zset（将分数设为一样的话，zset会按照字典序排，且内部有跳表优化查询），模糊匹配时才走数据库gin
