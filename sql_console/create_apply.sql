...
(
    ...
    target_type  boolean NOT NULL, -- 类型：friend 或 group
    target_id    bigint  NOT NULL, -- 目标ID：对方用户ID 或 群ID
...
-- 索引：查询发给我的申请（我的待办列表）
CREATE INDEX idx_apply_target ON apply (target_id, target_type) WHERE status = 'pending';
